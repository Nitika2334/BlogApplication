...
def test_update_post_failure(test_client, mocker):
    # Mock the update_post function to raise an exception
    mock_update_post = mocker.patch('App.api.wrapper.apis.update_post', side_effect=Exception('Failed to update post'))
    ...
    # Assertions
    assert response.status_code == 400
    assert response.json['message'] == 'Failed to update post'
    assert response.json['status'] is False
    assert response.json['type'] == 'custom_error'
    assert response.json['error_status']['error_code'] == '40005'
